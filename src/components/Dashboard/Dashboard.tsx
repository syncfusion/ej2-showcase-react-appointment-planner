...
export const Dashboard = () => {
    ...
    const getFilteredData = (startDate: Date, endDate: Date): Record<string, any>[] => {
        ...
        const filter: Record<string, any>[] = new DataManager({ json: hospitalData }).executeLocal(new Query().where(predicate));
        return filter;
    }
    const currentDayEvents: Record<string, any>[] = getFilteredData(dataService.selectedDate,
        addDays(new Date(dataService.selectedDate.getTime()), 1)) as Record<string, any>[];
    const currentViewEvents: Record<string, any>[] = getFilteredData(firstDayOfWeek,
    ...
    let gridData: Record<string, any>[] = getGridData();

    useEffect(() => {
        updateActiveItem('dashboard');
    }, []);

    const doctorColumnTemplate = (props: Record<string, any>): JSX.Element => {
